...
const SngProject = ({ project }) => {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-4 items-center justify-items-center gap-0 mx-5 bg-[#00a4d6] p-1 md:px-0 bg-opacity-40 rounded-md">
            ...
            <div className="w-full col-span-3 bg-black bg-opacity-95 md:bg-opacity-80  md:h-[340px] md:col-span-3 p-4 md:p-8 rounded-md border-2 border-white border-opacity-60 -mt-32 md:mt-0 md:-ms-5">
                <div className="flex flex-col md:flex-row items-center justify-start md:justify-between md:items-center md:-ms-2 w-11/12">
                    <h1 className="text-xl font-semibold text-[#00a4d6] flex gap-2 md:items-center"><BiSolidCategoryAlt /> {type}</h1>
                ...
                <div className="overflow-y-auto overflow-hidden h-52 md:h-[150px]">
                    <p style={{ whiteSpace: "pre-wrap" }}>{overview}</p>
                </div>
                <div className="flex flex-wrap gap-1 mt-1">
                    {
                        skills.map((skill, index) => (
                            <span key={index} className=" px-2 text-sm font-semibold text-black bg-gradient-to-r from-[#00a4d6] to-[#00aeff] rounded-full shadow-lg hover:from-[#479dcf] hover:to-[#00a2ff] focus:ring-2 focus:ring-blue-300 transition-all duration-300">{skill}</span>
                        ))
                    }
                </div>
            </div>
